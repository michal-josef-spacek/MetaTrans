...
$SIG{'__WARN__'} = sub { die $_[0]; };
eval {
	$obj->unset_dir_1_to_1('cze', 'ger');
};
$SIG{'__WARN__'} = $old;
like($EVAL_ERROR, qr{^language 'cze' not supported, not unsetting 'cze2ger' at},
	"Source language code 'cze' doesn't set.");

# Test.
$SIG{'__WARN__'} = sub {};
$ret = $obj->unset_dir_1_to_1('cze', 'ger');
$SIG{'__WARN__'} = $old;
is($ret, 0, "Return value if source language code 'cze' doesn't set.");
...
$SIG{'__WARN__'} = $old;
like($EVAL_ERROR, qr{^language 'ger' not supported, not unsetting 'cze2ger'},
...
$ret = $obj->unset_dir_1_to_1('cze', 'ger');
is($ret, 1, "Unset direction to 'ger' destination language code.");

# Test.
# TODO Bug.
$ret = $obj->unset_dir_1_to_1('cze', 'cze');
is($ret, 1, 'Unset direction to same language.');
